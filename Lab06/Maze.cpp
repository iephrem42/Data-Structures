/*
*  Names: Isaac Ephrem and Joshua Norman                                                                    
*  Date: 2-27-16 
* Assignment: Lab 6                                                          
*/
#include "Maze.h"
...
bool Maze::isSolved(Cell* curr_cell, StackLinked<Cell>* stack)
{
   ...
   int currentrow = curr_cell->getRow();
   int currentcolumn = curr_cell->getCol();

   //have you solved the maze? (check that we are at the bottom right maze location and that it is a SPACE
   if ((currentrow == height) && (currentcolumn == width) && (maze->getElement(currentrow, currentcolumn) == 1))  
   {
